...
record_field(Name, Value) ->
    record_field(Name, Value, none).

-spec record_field(syntaxTree(),
	'none' | syntaxTree(), 'none' | syntaxTree()) -> syntaxTree().

record_field(Name, Value, Type) ->
    tree(record_field, #record_field{name = Name, value = Value, type = Type}).
...
-spec record_field_type(syntaxTree()) -> 'none' | syntaxTree().

record_field_type(Node) ->
    (data(Node))#record_field.type.
...
subtrees(T) ->
	...
	false ->
		...
		record_field ->
			V = record_field_value(T),
			Type = record_field_type(T),
			if V == none, Type == none ->
			    [[record_field_name(T)]];
			V == none ->
				[[record_field_name(T)], [], [Type]];
			Type == none ->
				[[record_field_name(T)], [V]];
			true ->
				[[record_field_name(T)], [V], [Type]]
			end;
		record_index_expr ->
...
make_tree(record_field, [[N], [], [T]]) -> record_field(N, none, T);
make_tree(record_field, [[N], [E], [T]]) -> record_field(N, E, T);
...
unfold_record_field({typed_record_field, Field, Type}) ->
	unfold_record_field_1(Field, abstract(Type));
unfold_record_field(Field) ->
  unfold_record_field_1(Field, none).

unfold_record_field_1({record_field, Pos, Name}, Type) ->
    set_pos(record_field(Name, none, Type), Pos);
unfold_record_field_1({record_field, Pos, Name, Value}, Type) ->
    set_pos(record_field(Name, Value, Type), Pos).
